...
- GPU: NVIDIA GTX 1060 6GB+ or AMD Radeon RX (2019+)
...
NVIDIA_COMPATIBLE = {
    # GTX 10-series (1060 6GB+)
    "1060": 6,   # minimum 6GB variant
    "1070": 0, "1070 ti": 0, "1080": 0, "1080 ti": 0,
    # GTX 16-series
...
def check_gpu(model: str):
    ...
    for kw in NVIDIA_INCOMPATIBLE:
        if kw.replace(" ", "") in norm.replace(" ", ""):
            return False, f"NVIDIA {kw.upper()} is not supported. Minimum: GTX 1060 6GB."

    # Check NVIDIA compatible
    nvidia_match = re.search(r"(?:gtx|rtx)\s*(\d{4}(?:\s*(?:ti|super))*)", norm)
    if nvidia_match:
        gpu_key = nvidia_match.group(1).strip()
        # Check against compatible list
        base_num = re.match(r"\d{4}", gpu_key)
        if base_num:
            num = base_num.group()
            if num in NVIDIA_COMPATIBLE or gpu_key in NVIDIA_COMPATIBLE:
                # Special check for 1060 — must be 6GB
                if num == "1060":
                    if "3gb" in norm or "3 gb" in norm:
                        return False, "GTX 1060 3GB is not supported. Only GTX 1060 6GB+ is compatible."
                return True, f"NVIDIA GTX/RTX {gpu_key.upper()} — compatible."
            else:
                return False, f"NVIDIA {gpu_key.upper()} — not in compatibility list."
    ...
    for kw in AMD_GPU_INCOMPATIBLE_KEYWORDS:
        if kw.strip() and kw.strip() in norm:
            return False, f"AMD Radeon series too old. Minimum: RX 5500 XT (2019+)."

    # Intel integrated
    if "intel" in norm and ("uhd" in norm or "hd " in norm or "iris" in norm or "integrated" in norm):
        return False, "Integrated Intel graphics are not supported. A dedicated GPU is required."
